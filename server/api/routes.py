...
from server.core.answer_generation import (
...
async def get_llm_options():
    ...
    return StandardAPIResponse(
        ...
        data=[
            provider.title()
            for provider in settings.model_options.keys()
        ],
    )
...
async def get_vectorstore_count(model_provider: str):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Unable to read vector store.",
        ) from e
...
async def get_vectorstore_search(
    ...
    try:
        ...
        logger.info(
            f"Search requested with query: {request.query} "
        ...
        raise HTTPException(
            status_code=500,
            detail="Unable to search the document.",
        ) from e
...
async def chat(request: ChatRequest):
    try:
        ...
        logger.debug(
            f"Chat request for model: {request.model_name} "
            f"(provider: {request.model_provider})"
        )
        ...
        if (
            model_name
            not in settings.model_options[model_provider]["models"]
        ):
            logger.warning(
                "Invalid model name."
            )
