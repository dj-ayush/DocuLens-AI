    ...
    with st.spinner("Thinking..."):
      try:
        history = [
          ...
          for item in st.session_state.get("chat_history", [])[-10:]
        ]
        result = process_user_input(model_provider, model, question, history)
        output = result.get("answer", "") if isinstance(result, dict) else result
        st.markdown(output)
        if isinstance(result, dict) and result.get("grounded") and result.get("sources"):
          st.caption("Sources: " + "; ".join(
            f"{source.get('document_name', 'Document')} - page "
            f"{source.get('page_start') or source.get('page_number')}"
